...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  if (!file) {
    std::cerr << "write_courses_offered: Failed to open file " << COURSES_OFFERED_PATH << std::endl;
    return;
  }
  ...
  for (auto & course : all_courses) {
    if (course.quarter != "null") {
      ...
      not_offered.emplace_back(course);
    }
  }
...
void write_courses_not_offered(std::vector<Course> unlisted_courses) {
  ...
  if (!file) {
    std::cerr << "write_courses_not_offered: Failed to open file " << COURSES_OFFERED_PATH << std::endl;
    return;
  }
  file << "Title,Number of Units,Quarter" << std::endl;
